...
k2send::k2send ()
:  KMainWindow (0, "k2send"), m_printer (0)
{
...
k2send::setupActions ()
{
    ...
    action =
        new KAction (i18n ("Import Playlist"), cut, this, SLOT (importPlaylist ()), actionCollection (), "file_import");
    ...
    action =
        new KAction (i18n ("Remove"), "editdelete", cut, m_view, SLOT (slotRemoveBranch ()), actionCollection (),
                     "file_remove");

    action =
        new KAction (i18n ("Play"), "player_play", cut, m_view, SLOT (slotPlay ()), actionCollection (), "player_play");
    action =
        new KAction (i18n ("Stop"), "player_stop", cut, m_view, SLOT (slotStop ()), actionCollection (), "player_stop");
    action =
        new KAction (i18n ("Next"), "player_fwd", cut, m_view, SLOT (slotSkip ()), actionCollection (), "player_next");
    action =
        new KAction (i18n ("Loudness"), 0, cut, m_view, SLOT (slotLoudness ()), actionCollection (), "player_loudness");

    action =
        new KAction (i18n ("Clear"), "reload", cut, m_view, SLOT (slotPlaylistClear ()), actionCollection (),
                     "playlist_clear");

    action =
        new KAction (i18n ("Play"), "player_play", cut, m_view, SLOT (slotConsolePlay ()), actionCollection (),
                     "console_play");
    action =
        new KAction (i18n ("Stop"), "player_stop", cut, m_view, SLOT (slotConsoleStop ()), actionCollection (),
                     "console_stop");
    action =
        new KAction (i18n ("Clear"), "reload", cut, m_view, SLOT (slotConsoleClear ()), actionCollection (),
                     "console_clear");
    createGUI ();
...
void
k2send::dragEnterEvent (QDragEnterEvent * event)
{
    event->accept (KURLDrag::canDecode (event));
}

void
k2send::dropEvent (QDropEvent * event)
{
    KURL::List urls;
...
void
k2send::optionsConfigureKeys ()
{
    KKeyDialog::configure (actionCollection (), "k2sendui.rc");
}

void
k2send::optionsConfigureToolbars ()
{
...
void
k2send::changeStatusbar (const QString & text)
{
    statusBar ()->message (text);
}

void
k2send::changeCaption (const QString & text)
{
    setCaption (text);
}
