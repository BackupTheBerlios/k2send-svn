...
K2sendPlayer::K2sendPlayer(k2sendWidget *p,int vol , int filt) : loud_filt(filt) ,  volume(vol) , m_parent(p)
{
    is_connected = FALSE;
...
void K2sendPlayer::addCommand(K2sendPlayerCommand * cmd) {
    command_mutex.lock();
...
K2sendPlayerCommand *  K2sendPlayer::command() {
...
void K2sendPlayer::setAddr(QString & str){

    baddr = str ;
    kdDebug(200010) << "K2sendPlayer::setAddr " << baddr << endl;
}

bool K2sendPlayer::blueOpen()
{
...
bool K2sendPlayer::setReset(int hard) {
    char buffer[2];
    if (!is_connected)
        return FALSE;
...
/* set volume */
bool K2sendPlayer::setVolume(int volume, bool do_update) {
    char buffer[3];
    if (!is_connected)
        return FALSE;
...
bool K2sendPlayer::setLoudness(int filt) {
    char buffer[2];
    if (!is_connected)
        return FALSE;
...
unsigned long long K2sendPlayer::getTime(void) {
  struct timeval tv;
  gettimeofday(&tv, NULL);
  return((tv.tv_sec*1000ll) + (tv.tv_usec/1000ll));
}

void K2sendPlayer::updateStatus(int bytes_sent,int current_bytes_sent, int total,int play_length )
{
    ...
    if (diff)
        ...
        rate = 0;
    progress =(100*bytes_sent)/total;

    se = new K2sendStatusEvent(K2sendStatusEvent::EventProgress,NULL,progress);
    QApplication::postEvent( m_parent, se );
    se = new K2sendStatusEvent(K2sendStatusEvent::EventLength,NULL,progress);
    QApplication::postEvent( m_parent, se );
...
bool K2sendPlayer::bluePlay(QString * filename) {
    ...
    do {
        ...
        FD_ZERO(&rfds);  FD_SET(blue_sock, &rfds);
        FD_ZERO(&wfds);  FD_SET(blue_sock, &wfds);
        ...
        if(FD_ISSET(blue_sock, &rfds)) {
            ...
            kdDebug(200010) << "K2sendPlayer::bluePlay  got data "  << rc  << " " << buffer[0] << " " << buffer[1] << endl;
            /* handle commands */
            switch(buffer[0]) {
                case K2sendPlayerCommand::Buttons:
                    ...
                    if(buffer[1] & 8) {
                        QString msg = QString("Battery low");
                        se = new K2sendStatusEvent(K2sendStatusEvent::EventMessage,msg,2000);
                        QApplication::postEvent( m_parent, se );
                    }
                    break;
